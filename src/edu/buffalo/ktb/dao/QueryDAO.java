...
public class QueryDAO {
	...
	public List<Map<String, List<Integer>>> getResultPart3QueryOne(String dsName) {
		...
		try {
			...
			if (geneId != null && expList != null && expList.size() != 0) {
	...
	public double calculateJoinAndCorrelationAML(Map<String, List<Double>> patientExpressionMapALL,
	...
	public void getResultQueryPart3B(String informativeGenes) {
		...
		try {
			...
			String sql = "select dg.P_ID,pr.U_ID,mf.EXP from MICROARRAY_FACT mf, DISEASE ds, DIAGNOSIS dg, CLINICAL_SAMPLE cs,PROBE pr"
					...
					+ informativeGenes + ") order by dg.P_ID, pr.U_ID";
			pstmt = connection.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				if (patientGeneExpressionMapWithALL.containsKey(patientId)) {
					Map<String, Integer> geneExpressionMap = patientGeneExpressionMapWithALL.get(patientId);
					geneExpressionMap.put(uId, expression);
				} else {
					Map<String, Integer> geneExpressionMap = new HashMap<String, Integer>();
					geneExpressionMap.put(uId, expression);
					patientGeneExpressionMapWithALL.put(patientId, geneExpressionMap);
				}
			}
			rs.close();
			...
			while (rs.next()) {
				...
				String uId = rs.getString(2);
				int expression = rs.getInt(3);
				if (patientGeneExpressionMapWithNotALL.containsKey(patientId)) {
					Map<String, Integer> geneExpressionMap = patientGeneExpressionMapWithNotALL.get(patientId);
					geneExpressionMap.put(uId, expression);
				} else {
					Map<String, Integer> geneExpressionMap = new HashMap<String, Integer>();
					geneExpressionMap.put(uId, expression);
					patientGeneExpressionMapWithNotALL.put(patientId, geneExpressionMap);
				}
			}
			rs.close();
			// /test patient section
			sql = "select * from TEST_SAMPLE ts where ts.U_ID in (" + informativeGenes + ") order by ts.U_ID";
			pstmt = connection.prepareStatement(sql);
			// pstmt.setString(1, informativeGenes);
			rs = pstmt.executeQuery();
			rs.setFetchSize(5000);
			...
			for (int i = 0; i < 5; i++) {
				...
				patientGeneExpressionMap.put("test" + i, new HashMap<String, Integer>());
			...
			for (int i = 0; i < 5; i++) {
				double[] patientArray = new double[mapList.get(i).get("test" + i).keySet().size()];
				int k = 0;
				for (String uId : mapList.get(i).get("test" + i).keySet()) {
					patientArray[k] = Double.valueOf(mapList.get(i).get("test" + i).get(uId));
					k++;
				}
			...
			for (int i = 0; i < patientArrayList.size(); i++) {
				double[] correlationArray = new double[patientGeneExpressionMapWithALL.size()];
				int k = 0;
				for (String patientId : patientGeneExpressionMapWithALL.keySet()) {

					Map<String, Integer> geneExpressionMap = patientGeneExpressionMapWithALL.get(patientId);

					double[] trainingPatientArray = new double[geneExpressionMap.size()];
					int h = 0;
					for (String geneId : geneExpressionMap.keySet()) {
						trainingPatientArray[h] = geneExpressionMap.get(geneId);
						h++;
					}
					PearsonsCorrelation correlation = new PearsonsCorrelation();
					double corr = correlation.correlation(patientArrayList.get(i), trainingPatientArray);
			...
			for (int j = 0; j < patientArrayList.size(); j++) {
				...
				for (String patientId : patientGeneExpressionMapWithNotALL.keySet()) {
					int k = 0;
					Map<String, Integer> geneExpressionMap = patientGeneExpressionMapWithNotALL.get(patientId);
					correlationArrayB = new double[patientGeneExpressionMapWithNotALL.size()];
					double[] trainingPatientArray = new double[geneExpressionMap.size()];
					int h = 0;
					for (String geneId : geneExpressionMap.keySet()) {
						trainingPatientArray[h] = geneExpressionMap.get(geneId);
					...
					PearsonsCorrelation correlation = new PearsonsCorrelation();
					double corr = correlation.correlation(patientArrayList.get(j), trainingPatientArray);
